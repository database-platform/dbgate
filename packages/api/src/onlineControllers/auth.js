...
module.exports = {
  ...
  async oauthToken(params) {
    ...
    const allowedGroups = process.env.OAUTH_ALLOWED_GROUPS
      ? process.env.OAUTH_ALLOWED_GROUPS.split(',').map(group => group.toUpperCase().trim())
      : [];

    if (process.env.OAUTH_ALLOWED_GROUPS && !groups.some(group => allowedGroups.includes(group.toUpperCase().trim()))) {
      return {
        error: `Username ${login} does not belong to an allowed group`,
      };
    }
